...
function setNumVariants(n) { numVariants = Math.max(1, Math.min(24, parseInt(n) || 4)); log(`Number of variants set to ${numVariants}. Each can have its own camera angle.`); initVariants(); renderAngleSelectors(); renderGallery(); renderSeoTable(); }
...
function setupControls() {
  ...
  if (controls && !document.getElementById("num-variants-input")) {
    ...
    numDiv.innerHTML = `<label style="font-size:13px;color:#8e8e93">Number of variants (1–24 — you can assign a different camera angle to each): <input id="num-variants-input" type="number" min="1" max="24" value="${numVariants}" style="width:60px;margin-left:6px" onchange="setNumVariants(this.value)"></label>`;
